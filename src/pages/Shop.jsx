    ...
    return(
        ...
        <div className="flex flex-col justify-center items-center py-5 gap-8 max-w-[1440px] my-0 mx-auto ">
            ...
            <div className="flex gap-2 justify-center items-center flex-wrap">
                {categories.map((elm)=><Link className="w-[130px] h-[50px] px-1 flex items-center justify-center bg-[#fff] text-[#000] rounded-3xl text-center text-wrap  hover:bg-black hover:text-[#7CFC00]" key={elm.slug} to={`shop/category/${elm.slug}`}>{elm.name}</Link>)}
            </div>
